...
{
    // CHANGED: Use SkiLocomotionV10 instead of LocomotionTechnique
    public SkiLocomotionV10 skiLocomotion;
    ...
    void Update()
    {
        if (isStageChange)
        {
            ...
            // CHANGED: Read stage from skiLocomotion instead of locomotionTech
            string currentStage = skiLocomotion != null ? skiLocomotion.stage : "";
            
            if (currentStage == startBanner.name)
            {
                parkourStart = true;
                ...
                Debug.Log("[ParkourCounter] Stage: START -> First, Coins activated!");
            }
            else if (currentStage == firstBanner.name)
            {
                firstBanner.SetActive(false);
                firstCoins.SetActive(false);
                objIX1.SetActive(false);
                ...
                Debug.Log("[ParkourCounter] Stage: First -> Second");
            }
            else if (currentStage == secondBanner.name)
            {
                secondBanner.SetActive(false);
                secondCoins.SetActive(false);
                objIX2.SetActive(false);
                ...
                Debug.Log("[ParkourCounter] Stage: Second -> Final");
            }
            else if (currentStage == finalBanner.name)
            {
                parkourStart = false;
                finalCoins.SetActive(false);
                objIX3.SetActive(false);
            ...
            {
                Debug.LogWarning($"[ParkourCounter] Unknown stage: '{currentStage}'");
            }
        }
